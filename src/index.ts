...
export function isInSubnet(address: string, subnetOrSubnets: string | string[]): boolean {
  ...
  if (isIPv6(address) && IPv6.isIPv4MappedAddress(address)) {
    return (
      IPv6.isInSubnet(
        address,
        subnetOrSubnets.filter(subnet => isIPv6(subnet.split('/')[0]))
      ) ||
      IPv4.isInSubnet(
        IPv6.extractMappedIpv4(address),
        subnetOrSubnets.filter(subnet => isIPv4(subnet.split('/')[0]))
      )
    );
  }
...
export { IPv4, IPv6, isIPv4, isIPv6 };
